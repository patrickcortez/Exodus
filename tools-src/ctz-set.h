...
#define CTZ_PAGE_SIZE 4096
#define CTZ_INVALID_PAGE_ID UINT32_MAX

#define DB_MAGIC 0x43545A31
#define DB_ROOT_PAGE_ID 1
...
static const DiskPtr DISK_PTR_NULL = { 0xFFFFFFFF, 0xFFFF };

#define DISK_PTR_EQ(a, b) ((a).page_id == (b).page_id && (a).slot_id == (b).slot_id)

typedef uint32_t PageId;

typedef struct {
    uint8_t data[CTZ_PAGE_SIZE];
} Page;



// Opaque handles for the new storage engine
...
// Opaque handles moved to ctz-set.c
