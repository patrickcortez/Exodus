/* tools/decrypt.c
 *
 * Professional-grade file decryption using Aethel-128 in CBC mode with HMAC-SHA256.
 * Verifies HMAC tag for integrity and authenticity before decryption.
 *
 * This is the complete, self-contained file with all necessary functions.
 *
 * Usage: decrypt <key_file> <input_ciphertext_file> <output_plaintext_file>
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
static void inv_mix_columns(state_t* state) {
    ...
    for (int j = 0; j < 4; j++) {
        ...
        (*state)[0][j] = gmul(t[0], 0x0e) ^ gmul(t[1], 0x0b) ^ gmul(t[2], 0x0d) ^ gmul(t[3], 0x09);
        (*state)[1][j] = gmul(t[0], 0x09) ^ gmul(t[1], 0x0e) ^ gmul(t[2], 0x0b) ^ gmul(t[3], 0x0d);
        (*state)[2][j] = gmul(t[0], 0x0d) ^ gmul(t[1], 0x09) ^ gmul(t[2], 0x0e) ^ gmul(t[3], 0x0b);
        (*state)[3][j] = gmul(t[0], 0x0b) ^ gmul(t[1], 0x0d) ^ gmul(t[2], 0x09) ^ gmul(t[3], 0x0e);
    }
}

static void aethel_decrypt_block(uint8_t* block, const uint8_t round_keys[NUM_ROUNDS + 1][AETHEL_BLOCK_SIZE]) {
    state_t state;
...
int main(int argc, char *argv[]) {
    ...
    if (fread(aethel_key, 1, AETHEL_KEY_SIZE, key_f) != AETHEL_KEY_SIZE || fread(hmac_key, 1, HMAC_KEY_SIZE, key_f) != HMAC_KEY_SIZE) {
        fprintf(stderr, "Key file must be at least %d bytes.\n", AETHEL_KEY_SIZE + HMAC_KEY_SIZE); fclose(key_f); return 1;
    }
    fclose(key_f);
    ...
    // 2. Read entire encrypted payload and separate IV, ciphertext, and tag
    fseek(in_f, 0, SEEK_END);
    long file_size = ftell(in_f);
    fseek(in_f, 0, SEEK_SET);
    if (file_size < AETHEL_BLOCK_SIZE + SHA256_DIGEST_SIZE) { fprintf(stderr, "Invalid ciphertext file: too short.\n"); fclose(in_f); return 1; }
    
    long ct_mac_size = file_size - AETHEL_BLOCK_SIZE - SHA256_DIGEST_SIZE;
    if (ct_mac_size < 0 || ct_mac_size % AETHEL_BLOCK_SIZE != 0) { fprintf(stderr, "Invalid ciphertext file: corrupted size.\n"); fclose(in_f); return 1; }

    uint8_t* mac_buffer = malloc(AETHEL_BLOCK_SIZE + ct_mac_size);
    if (!mac_buffer) { fprintf(stderr, "Failed to allocate memory for verification.\n"); fclose(in_f); return 1; }
    
    uint8_t received_tag[SHA256_DIGEST_SIZE];
    if (fread(mac_buffer, 1, AETHEL_BLOCK_SIZE + ct_mac_size, in_f) != (size_t)(AETHEL_BLOCK_SIZE + ct_mac_size)) {
        fprintf(stderr, "Error reading ciphertext payload.\n"); free(mac_buffer); fclose(in_f); return 1;
    }
    if (fread(received_tag, 1, SHA256_DIGEST_SIZE, in_f) != SHA256_DIGEST_SIZE) {
        fprintf(stderr, "Error reading HMAC tag.\n"); free(mac_buffer); fclose(in_f); return 1;
    }
    fclose(in_f);
    
    // 3. VERIFY HMAC BEFORE DECRYPTION
    uint8_t calculated_tag[SHA256_DIGEST_SIZE];
    hmac_sha256(hmac_key, HMAC_KEY_SIZE, mac_buffer, AETHEL_BLOCK_SIZE + ct_mac_size, calculated_tag);

    int diff = 0;
    for(int i = 0; i < SHA256_DIGEST_SIZE; i++) diff |= received_tag[i] ^ calculated_tag[i];
    if (diff != 0) {
        fprintf(stderr, "DECRYPTION FAILED: Message authentication failed. The file has been tampered with or the key is incorrect.\n");
        free(mac_buffer);
        return 1;
    }

    // 4. HMAC is valid, now proceed with CBC decryption
    FILE *out_f = fopen(argv[3], "wb");
    if (!out_f) { perror("Could not open output file"); free(mac_buffer); return 1; }

    uint8_t round_keys[NUM_ROUNDS + 1][AETHEL_BLOCK_SIZE];
    key_expansion(aethel_key, round_keys);

    uint8_t* iv = mac_buffer;
    uint8_t* ciphertext = mac_buffer + AETHEL_BLOCK_SIZE;
    uint8_t prev_cipher_block[AETHEL_BLOCK_SIZE];
    memcpy(prev_cipher_block, iv, AETHEL_BLOCK_SIZE);

    for(long i = 0; i < ct_mac_size; i += AETHEL_BLOCK_SIZE) {
        uint8_t current_cipher_block[AETHEL_BLOCK_SIZE];
        memcpy(current_cipher_block, ciphertext + i, AETHEL_BLOCK_SIZE);
        
        uint8_t decrypted_block[AETHEL_BLOCK_SIZE];
        memcpy(decrypted_block, current_cipher_block, AETHEL_BLOCK_SIZE);
        aethel_decrypt_block(decrypted_block, round_keys);

        for(int j=0; j < AETHEL_BLOCK_SIZE; j++) decrypted_block[j] ^= prev_cipher_block[j];

        memcpy(prev_cipher_block, current_cipher_block, AETHEL_BLOCK_SIZE);

        if (i + AETHEL_BLOCK_SIZE >= ct_mac_size) {
            uint8_t padding_val = decrypted_block[AETHEL_BLOCK_SIZE - 1];
            if (padding_val > 0 && padding_val <= AETHEL_BLOCK_SIZE) {
                fwrite(decrypted_block, 1, AETHEL_BLOCK_SIZE - padding_val, out_f);
            } else {
                 fwrite(decrypted_block, 1, AETHEL_BLOCK_SIZE, out_f);
            }
        } else {
            fwrite(decrypted_block, 1, AETHEL_BLOCK_SIZE, out_f);
        }
    ...
    free(mac_buffer);
    fclose(out_f);
    printf("Decryption complete and message authenticated successfully.\n");
    return 0;
}