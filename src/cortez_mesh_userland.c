/*
 * cortez-mesh-userland.c
 * Userland-only (non-root) implementation of the Cortez Mesh.
 * Uses POSIX shared memory (shm_open) instead of the cortez_tunnel driver.
 *
 * Compile Command for library:
 * gcc -Wall -Wextra -O2 -c cortez-mesh-userland.c -o cortez-mesh-userland.o -pthread -lrt
 */
...
#include <limits.h>
#include <pthread.h>
#include <dirent.h> // For mesh discovery

// --- USERLAND-SPECIFIC INCLUDES ---
#include <sys/mman.h>
#include <sys/stat.h>
...
// --- Defines (Copied from cortez-mesh.c) ---
...
// --- Internal Structs (Modified for Userland) ---

struct cortez_ch {
    int fd;                 // File descriptor from shm_open()
    char name[64];
    char shm_name[128];     // --- NEW: Name for shm_open/shm_unlink
    void* shm_base;
    size_t shm_size;
...
// All other structs are identical to cortez-mesh.c
typedef struct {
    uint64_t magic;
    uint32_t total_len;
} CortezJumpHeader;
...
static const char* internal_strerror(int err) {
    switch (err) {
        ...
        case CORTEZ_E_IOCTL_FAILED: return "Filesystem/SHM operation failed";
...
/**
 * @brief RE-IMPLEMENTED: Joins a channel using POSIX shared memory.
 */
cortez_ch_t* cortez_join(const char* channel_name, const cortez_options_t* options) {
    ...
    // Create the POSIX SHM name (must start with /)
    snprintf(ch->shm_name, sizeof(ch->shm_name), "/cortez_shm_%s", channel_name);

    int is_creator = 0;
    
    // --- Kernel 'ioctl(TUNNEL_CREATE)' is replaced by 'shm_open' ---
    if (options->create_policy == CORTEZ_CREATE_ONLY) {
        ch->fd = shm_open(ch->shm_name, O_RDWR | O_CREAT | O_EXCL, 0660);
        if (ch->fd < 0) {
            set_error(ch, (errno == EEXIST) ? CORTEZ_E_CHAN_EXISTS : CORTEZ_E_IOCTL_FAILED);
            free(ch); return NULL;
        }
        is_creator = 1;
        ch->shm_size = options->size;
    
    // --- Kernel 'ioctl(TUNNEL_CONNECT)' is replaced by 'shm_open' ---
    } else if (options->create_policy == CORTEZ_JOIN_ONLY) {
        ch->fd = shm_open(ch->shm_name, O_RDWR, 0660);
        if (ch->fd < 0) {
        ...
        if (ch->fd >= 0) {
            ...
            // It already exists, join it
            ch->fd = shm_open(ch->shm_name, O_RDWR, 0660);
            if (ch->fd < 0) {
                set_error(ch, CORTEZ_E_IOCTL_FAILED); // Failed to join
                free(ch); return NULL;
            }
            is_creator = 0;
            ch->shm_size = 4096; // Map one page to read header
        } else {
    ...
    if (is_creator) {
        if (ftruncate(ch->fd, (off_t)ch->shm_size) != 0) {
            set_error(ch, CORTEZ_E_IOCTL_FAILED);
            close(ch->fd); shm_unlink(ch->shm_name); free(ch); return NULL;
        }
    }

    ch->shm_base = mmap(NULL, ch->shm_size, PROT_READ | PROT_WRITE, MAP_SHARED, ch->fd, 0);
    if (ch->shm_base == MAP_FAILED) {
        set_error(ch, CORTEZ_E_SHM_MAP_FAILED);
        close(ch->fd);
        if (is_creator) shm_unlink(ch->shm_name);
        free(ch); return NULL;
    }
    ch->header = (CortezChannelHeader*)ch->shm_base;

    // --- Header init / re-map (same as before) ---
    if (is_creator) {
        ch->is_owner = 1;
...
void cortez_abort_write_zc(cortez_write_handle_t* handle) {
    ...
    cortez_abort_write(handle->ch, handle->tx);
    free(handle);
}
...
int cortez_get_stats(cortez_ch_t* ch, cortez_stats_t* stats) {
    ...
    stats->messages_written = __atomic_load_n(&h->messages_written, __ATOMIC_RELAXED);
    stats->messages_read = __atomic_load_n(&h->messages_read, __ATOMIC_RELAXED);
    stats->bytes_written = __atomic_load_n(&h->bytes_written, __ATOMIC_RELAXED);
    stats->bytes_read = __atomic_load_n(&h->bytes_read, __ATOMIC_RELAXED);
    stats->write_contention_count = __atomic_load_n(&h->write_contention_count, __ATOMIC_RELAXED);
    stats->channel_recovered_count = __atomic_load_n(&h->channel_recovered_count, __ATOMIC_RELAXED);
    stats->active_connections = __atomic_load_n(&h->active_connections, __ATOMIC_RELAXED);
    stats->owner_pid = h->owner_pid;
    stats->buffer_capacity = h->buffer_capacity;
    stats->buffer_bytes_used = get_read_space(h, head, tail);
...
static void remove_peer(cortez_mesh_t* mesh, pid_t pid) {
    ...
    while (*pptr) {
        ...
        if (entry->info.pid == pid) {
            *pptr = entry->next;
            char process_name[64];
            get_process_name_by_pid(entry->info.pid, process_name, sizeof(process_name));
            printf("[Mesh] Peer '%s' left/timed out: %d\n", process_name, entry->info.pid);
...
static void* housekeeper_thread_main(void* arg) {
    ...
    int64_t last_heartbeat_sent_ns = 0;
    ...
    while (mesh->housekeeper_running) {
        ...
        while ((msg = cortez_read(mesh->registry_ch, 0)) != NULL) {
            ...
            if (peer_info->pid == mesh->self_info.pid) {
                cortez_msg_release(mesh->registry_ch, msg);
                continue;
            }
        ...
        if (last_heartbeat_sent_ns == 0 || now_ns - last_heartbeat_sent_ns >= (int64_t)HEARTBEAT_INTERVAL_SEC * 1000000000LL) {
        ...
        while(*pptr) {
            ...
            if (now_ns - entry->last_heartbeat > (int64_t)PEER_TIMEOUT_SEC * 1000000000LL) {
                 *pptr = entry->next;
                 char process_name[64];
                 get_process_name_by_pid(entry->info.pid, process_name, sizeof(process_name));
                 printf("[Mesh] Peer '%s' timed out: %d\n", process_name, entry->info.pid);
                 if (entry->comm_channel) cortez_leave(entry->comm_channel);
                 free(entry);
            } else {
                pptr = &(*pptr)->next;
...
cortez_mesh_t* cortez_mesh_init(const char* node_name, const cortez_options_t* options) {
    ...
    while ((stale_msg = cortez_read(mesh->registry_ch, 0)) != NULL) {
...
pid_t cortez_mesh_find_peer_by_name(cortez_mesh_t* mesh, const char* name) {
    ...
    struct dirent* entry;
    char prefix[128];
    snprintf(prefix, sizeof(prefix), "cortez_shm_%s-", name);
    size_t prefix_len = strlen(prefix);

    while ((entry = readdir(dir)) != NULL) {
        if (strncmp(entry->d_name, prefix, prefix_len) == 0) {
            pid_t pid = (pid_t)atol(entry->d_name + prefix_len);
...
void cortez_mesh_list_peers(cortez_mesh_t* mesh) {
    ...
    for (cortez_peer_t* p = mesh->peer_list; p != NULL; p = p->next) {
        char process_name[64];
        get_process_name_by_pid(p->info.pid, process_name, sizeof(process_name));
        printf("  - PID: %d, Name: %s, Inbox: %s\n", p->info.pid, process_name, p->info.inbox_channel_name);
    }
    printf("----------------------------------\n");
    pthread_mutex_unlock(&mesh->peer_list_mutex);
}
